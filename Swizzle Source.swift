...
do {
    let i = try Interpreter(code: code, debug: false, stackTrace: false)
    ...
    let s = time < 1 ? Double((time * 100).rounded()) / 100 : time.rounded(.up)
    let m = "Execution took \(s) second\(s == 1 ? "" : "s")."
    print(String(repeating: "-", count: m.count))
    print(m)
}
catch {
    print(error)
